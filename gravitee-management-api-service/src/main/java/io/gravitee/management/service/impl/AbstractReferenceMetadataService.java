...
import io.gravitee.management.service.exceptions.ApplicationMetadataNotFoundException;
...
public abstract class AbstractReferenceMetadataService {
    ...
    public List<ReferenceMetadataEntity> findAllByReference(final MetadataReferenceType referenceType, final String referenceId) {
        try {
            ...
            List<ReferenceMetadataEntity> allMetadata = new ArrayList<>();
    ...
    public ReferenceMetadataEntity findByIdAndReference(final String metadataId, final MetadataReferenceType referenceType, final String referenceId) {
        LOGGER.debug("Find metadata by id {} and reference {} / {}", metadataId, referenceType, referenceId);
        final List<ReferenceMetadataEntity> allMetadata = findAllByReference(referenceType, referenceId);
    ...
    public void delete(final String metadataId, final MetadataReferenceType referenceType, final String referenceId) {
    ...
    public ReferenceMetadataEntity create(final NewReferenceMetadataEntity metadataEntity,
        ...
        final Optional<ReferenceMetadataEntity> optionalMetadata = findAllByReference(referenceType, referenceId).stream()
    ...
    public ReferenceMetadataEntity update(final UpdateReferenceMetadataEntity metadataEntity,
                                          final MetadataReferenceType referenceType, final String referenceId) {
        checkReferenceMetadataFormat(metadataEntity.getFormat(), metadataEntity.getValue(), referenceType, referenceId);
        try {
            final List<MetadataEntity> defaultMetatata = metadataService.findAllDefault();
            final Optional<MetadataEntity> optDefaultMetadata =
                    defaultMetatata.stream().filter(metadata -> metadata.getKey().equals(metadataEntity.getKey())).findAny();
